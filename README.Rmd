...
## By number of male or female authors, regardless of how many other authors of the opposite sex are on a preprint
...
p11 <- ggplot(data=all.long, aes(fill=year, x=`Male or female authors (no.)`, y=as.numeric(number)))+labs(fill="Year")+ylab("Preprints (no.)")+geom_bar(position="dodge", stat="identity")+theme_cowplot()+scale_fill_viridis(option = "cividis", discrete=TRUE)+facet_wrap(~gender)
p11
```
...
df.b.2020$author.n <- str_count(df.b.2020$authors, pattern = "\\;")+1 #Count author number
df.b.2019$author.n <- str_count(df.b.2019$authors, pattern = "\\;")+1 

max(df.b.2020$author.n) #Max author number 
max(df.b.2019$author.n) 

df.b.2020$first.author <- trimws(sapply(strsplit(as.character(df.b.2020$authors), ";", fixed=TRUE), head, 1)) #Extract first author 
df.b.2019$first.author <- trimws(sapply(strsplit(as.character(df.b.2019$authors), ";", fixed=TRUE), head, 1))

df.b.2020$cor.author.first.name <- sapply(strsplit(df.b.2020$author_corresponding, " "), head, 1)
df.b.2019$cor.author.first.name <- sapply(strsplit(df.b.2019$author_corresponding, " "), head, 1)

```

```{r}
gender.2020 <- NULL
gender.2020 <- gender(df.b.2020$cor.author.first.name, method = "ssa")
gender.2020 <- unique(gender.2020[ , c(1,2,4)])
df.b.2020 <- merge(df.b.2020, gender.2020, by.x = "cor.author.first.name",  by.y ="name", all = TRUE)

gender.2019 <- NULL
gender.2019 <- gender(df.b.2019$cor.author.first.name, method = "ssa")
gender.2019 <- unique(gender.2019[ , c(1,2,4)])
df.b.2019 <- merge(df.b.2019, gender.2019, by.x = "cor.author.first.name",  by.y ="name", all = TRUE)

bsum.2020 <- df.b.2020 %>% group_by(gender) %>% summarize(n=n())
bsum.2019 <- df.b.2019 %>% group_by(gender) %>% summarize(n=n())

df.b.2019$date <- as.Date(df.b.2019$date)
df.b.2020$date <- as.Date(df.b.2020$date)

sum.b.2019 <- subset(df.b.2019, !is.na(gender)) %>% group_by(date) %>% summarize(n=n(), female.n = length(which(gender =="female")), female.per = female.n/n)
sum.b.2019 <- ungroup(sum.b.2019)
sum.b.2019$year <- "2019"
sum.b.2019$date.noyear <- paste0(month(sum.b.2019$date), "/", day(sum.b.2019$date))

sum.b.2020 <- subset(df.b.2020, !is.na(gender)) %>% group_by(date) %>% summarize(n=n(), female.n = length(which(gender =="female")), female.per = female.n/n)
sum.b.2020 <- ungroup(sum.b.2020)
sum.b.2020$year <- "2020"
sum.b.2020$date.noyear <- paste0(month(sum.b.2020$date), "/", day(sum.b.2020$date))

summaryb <- rbind(sum.b.2019, sum.b.2020)
summaryb$anydate <- anydate(summaryb$date.noyear)
summaryb$weekday <- weekdays(summaryb$date)

p12 <- ggplot(data=summaryb, aes(x=anydate, y=female.per, color=year))+geom_point()+ylab("Female (%)")+xlab("Date")+labs(color="Year")+geom_smooth(method="lm", se=FALSE)+ggtitle("Corresponding authors")+theme_cowplot()
p12

save_plot("plot3.png", p12, base_height=2.5, base_width=6)
```