...
I began by scraping all records for March 15-April 15, 2020, during the COVID-19 pandemic, and for the same date range in 2019 and 2018. Then, I scraped all the data for January 1 to April 22, 2020. I scraped the data in batches, as recommended in the aRxiv package tutorial. 
...
There are `r total.preprints` preprints in the arXiv dataset for 2019-2020, with a total of `r format(total.authors, scientific=FALSE)` non-unique authors. I inferred the gender of `r total.authors.with.gender` authors, or `r per.gender`%, with the rest omitted from subsequent analyses. 
...
How many arXiv preprints were sole-authored by a female versus a male academic in Mar/Apr, 2020, compared to the same dates last year? 
...
The number of male authors of arXiv preprints has increased quite a bit through early 2020, while the number of female authors of arXiv preprints has plateaued (March) or increasingly more modestly (April) during the pandemic.

Percent changes do not appear on the figure, but arXiv preprints with female corresponding authors have increased `r a.fem.per.increase`% between Jan/Feb 2020 and Mar/Apr 2020, while arXiv preprints with male corresponding authors have increased `r a.m.per.increase`% over the same period. 

The same data, but in tabular form:

```{r, arXiv early 2020 table, by month}
#Or in tabular form
...
I then inferred the gender of corresponding authors of bioRxiv preprints, as above. Note that the bioRxiv API only returns first names for corresponding authors, and not for all authors. (Unfortunately.)
...
There are `r total.b.preprints` preprints in the bioRxiv dataset for 2019-2020, each with a single corresponding author. I inferred the gender of `r total.b.authors.with.gender` corresponding authors, or `r per.b.gender`%, with the rest omitted from subsequent analyses. 
...
The number of male authors of bioRxiv preprints has increased steadily through early 2020, while the number of female authors of bioRxiv preprints has increased only slightly. 

Again the percent change does not appear on the figure, but bioRxiv preprints with female corresponding authors have increased `r fem.per.increase`% between Jan/Feb 2020 and Mar/Apr 2020, while bioRxiv preprints with male corresponding authors have increased `r m.per.increase`% over the same period. 

The same data, but in tabular form:

```{r, bioRxiv early 2020 table, by month}
#Or in tabular form
...
I also made an additional, simpler figure to accompany a story I wrote for *The Conversation.*

```{r The Conversation figure, message=FALSE, warning=FALSE, fig.width=8, fig.height=8, dpi=300}
