...
p3 <- ggplot(data=subset(arxiv.w.long, year(week) == 2020), aes(color=gender, y=n, x=week))+geom_rect(aes(xmin=as.Date(covid.start), xmax=as.Date(Inf), ymin=-Inf, ymax=Inf), alpha=0.2, fill="gainsboro", color="gainsboro")+geom_point()+geom_smooth(data=subset(arxiv.w.long, year(week) == 2020 & week != "2020-04-12"), aes(color=gender, y=n, x=week), se=FALSE)+theme_cowplot()+ggtitle("arXiv: 2020")+xlab("Date")+ylab("Authors (no.)")+labs(color="Gender")
p3
```
...
p8 <- ggplot(data=arxiv.m.long, aes(color=gender, y=pubs.per.day, x=month))+geom_point()+geom_line()+theme_cowplot()+ggtitle("aRxiv: 2020")+xlab("Month")+ylab("Preprint authors per day (no.)")+labs(color="Gender")
p8
```
...
p6 <- ggplot(data=subset(biorxiv.w, year(week) == 2020), aes(color=gender, y=sum, x=week))+geom_rect(aes(xmin=as.Date(covid.start), xmax=as.Date(Inf), ymin=-Inf, ymax=Inf), alpha=0.2, fill="gainsboro", color="gainsboro")+geom_point()+geom_smooth(span=1, se=FALSE)+theme_cowplot()+ggtitle("bioRxiv: 2020")+xlab("Date")+ylab("Authors (no.)")+labs(color="Gender")
p6
```
...
p7 <- ggplot(data=biorxiv.m, aes(color=gender, y=pubs.per.day, x=month))+geom_point()+geom_line()+theme_cowplot()+ggtitle("bioRxiv: 2020")+xlab("Month")+ylab("Preprint authors per day (no.)")+labs(color="Gender")
p7
```
